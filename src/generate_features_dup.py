...
features_head = ['Location', 'ID', 'SV_LEN', 'CADD_RAW', 'CADD_PHRED', 'GDI_score', 'GDI_Phred', 'Lof_score', 'MCAP_score',
                    'DamagePredCount_mean', 'vest_score_mean', 'MVP_score_mean', 'MPC_score_mean', 'DANN_score_mean',
                    'GenoCanyon_score_mean', 'Integrated_fitCons_score_mean', 'GM12878_fitCons_score_mean',
                    'H1_hESC_fitCons_score_mean', 'HUVEC_fitCons_score_mean', 'LINSIGHT_mean', 'GERP_NR_mean',
                    'GERP_RS_mean', 'PhyloP100way_vertebrate_mean', 'PhyloP30way_mammalian_mean',
                    'PhyloP17way_primate_mean', 'PhastCons100way_vertebrate_mean', 'PhastCons30way_mammalian_mean',
                    'PhastCons17way_primate_mean', 'RVIS_ExAC_005_1', 'RVIS_ExAC_005_2', 'REVEL_score', 'Roadmap_SRS004212',
                    'E003_DNase', 'E003_H3K27ac', 'E003_H3K27me3', 'E003_H3K36me3', 'E003_H3K4me1_signal', 'E003_H3K4me3_signal',
                    'E003_H3K9me3_signal', 'E003_Methylation_signal', 'ENCFF225MAO_signal', 'EncodeUwRepliSeqGm12878WaveSignal_signal',
                    'phyloP100way_signal', '3utr_overlap', '5utr_overlap', 'CDS_overlap', 'Prom_overlap', 'SS_overlap', 'TAD_overlap',
                    'sensitive_overlap', 'conserved_overlap', 'Heterochromatin_overlap',
                    'All_disease_causing_High', 'All_disease_causing_Low', 'All_disease_causing_Medium',
                    'All_Mendelian_High', 'All_Mendelian_Low', 'All_Mendelian_Medium',
                    'Mendelian_AD_High', 'Mendelian_AD_Low', 'Mendelian_AD_Medium',
                    'Mendelian_AR_High', 'Mendelian_AR_Low', 'Mendelian_AR_Medium',
                    'all_PID_High', 'all_PID_Low', 'all_PID_Medium',
                    'PID_AD_High', 'PID_AD_Low', 'PID_AD_Medium',
                    'PID_AR_High', 'PID_AR_Low', 'PID_AR_Medium',
                    'All_cancer_High', 'All_cancer_Low', 'All_cancer_Medium',
                    'Cancer_recessive_High', 'Cancer_recessive_Low', 'Cancer_recessive_Medium',
                    'Cancer_dominant_High', 'Cancer_dominant_Low', 'Cancer_dominant_Medium',
                    'ExonicFunc.refGene_frameshift_insertion', 'ExonicFunc.refGene_frameshift_substitution',
                    'ExonicFunc.refGene_nonframeshift_insertion', 'ExonicFunc.refGene_startloss', 'ExonicFunc.refGene_stopgain',
                    'ExonicFunc.refGene_stoploss', 'ExonicFunc.refGene_unknown',
                    'SIFT_pred_.', 'SIFT_pred_D', 'SIFT_pred_T',
                    'SIFT4G_pred_.', 'SIFT4G_pred_D', 'SIFT4G_pred_T',
...
def cadd_score(chrom, pos, sv_len, ref_length, snpINFO, CADD_dir, step):
    cadd_files_list = get_sv_score_file_list(chrom, pos + sv_len, pos + 2 * sv_len, step, ref_length, CADD_dir, 'cadd')
    scope4currentsv = []
        ...
        with open(cadd_file, 'r') as read_cadd:
            ...
            while not isFinished:
                ...
                if line:
                    ...
                    temp = []
                    if list_text[0] == str(chrom) and int(list_text[1]) >= pos + sv_len + 1 and int(list_text[2]) < pos + 2 * sv_len +1:
                    ...
                    elif list_text[0] == str(chrom) and int(list_text[2]) >= pos + 2 * sv_len + 1:
                        isFinished = True
                else:
                    isFinished = True
    count = 0
...
def dbnsfp(chrom, pos, sv_len, ref_length, snpINFO, dbnsfp_dir, step):
    scope4currentsv = []
    dbnsfp_files_list = get_sv_score_file_list(chrom, pos + sv_len, pos + 2 * sv_len, step, ref_length, dbnsfp_dir, 'dbnsfp')
        ...
        with open(dbnsfp_file, 'r') as read_dbnsfp:
            while True:
                ...
                if line:
                    list_text = line.split()
                    temp = []
                    if str(list_text[0]) == str(chrom) and int(list_text[1]) >= pos + sv_len + 1 and int(
                            list_text[2]) < pos + 2 * sv_len + 1:
...
def mcap(chrom, pos, sv_len, ref_length, snpINFO, mcap_dir, step):
    scope4currentsv = []
    mcap_files_list = get_sv_score_file_list(chrom, pos + sv_len, pos + 2 * sv_len, step, ref_length, mcap_dir, 'mcap')
...
def revel(chrom, pos, sv_len, ref_length, snpINFO, revel_dir, step):
    revel_files_list = get_sv_score_file_list(chrom, pos + sv_len, pos + 2 * sv_len, step, ref_length, revel_dir, 'revel')
    scope4currentsv = []
...
def onehot_exonicFunc(exonicFunc_refGene):
    ExonicFuncRefGene = [0, 0, 0, 0, 0, 0, 1]
    if exonicFunc_refGene == 'frameshift_insertion':
        ExonicFuncRefGene = [1, 0, 0, 0, 0, 0, 0]
    elif exonicFunc_refGene == 'frameshift_substitution':
        ExonicFuncRefGene = [0, 1, 0, 0, 0, 0, 0]
    elif exonicFunc_refGene == 'nonframeshift_insertion':
        ExonicFuncRefGene = [0, 0, 1, 0, 0, 0, 0]
    elif exonicFunc_refGene == 'startloss':
        ExonicFuncRefGene = [0, 0, 0, 1, 0, 0, 0]
    elif exonicFunc_refGene == 'stopgain':
        ExonicFuncRefGene = [0, 0, 0, 0, 1, 0, 0]
    elif exonicFunc_refGene == 'stoploss':
        ExonicFuncRefGene = [0, 0, 0, 0, 0, 1, 0]
...
def extract_features(reference, cadd_dir, mcap_dir, dbnsfp_dir, revel_dir, gene_dir, bw_dir, overlap_dir, step, variationINFO):
    ...
    try:
        ...
        if not str(chrom).startswith('chr'):
            tmpchrom = 'chr' + chrom
        snpREF = fasta.fetch(tmpchrom, pos + sv_len, pos + 2 * sv_len)
        ref_length = fasta.get_reference_length(tmpchrom)
        snpALT = alt[1:]
        snpINFO = []

        for i in range(len(snpREF)):
            snppos = pos + i + sv_len + 1
            snpref = snpREF[i]
            snpalt = snpALT[i]
            temp4snp = []
        ...
        bwFeatures_scores = featuresInbwFiles(bw_dir, chrom, pos + sv_len + 1, pos + 2 * sv_len + 1)
        for i in range(len(bwFeatures_scores)):
            featuresValue4currentSV.append(round(bwFeatures_scores[i], 6))

        overlapFeatures = featuresForOverlap(overlap_dir, chrom, pos + sv_len + 1, pos + 2 * sv_len + 1)
        ...
        print('\tchr' + str(chrom) + ', pos ' + str(pos) + ' duplication variation features calculation completed')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a feature matrix from duplication SV data')
