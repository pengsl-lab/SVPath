...
features_head = ['Location', 'ID', 'SV_LEN', 'CADD_RAW', 'CADD_PHRED', 'GDI_score', 'GDI_Phred', 'Lof_score', 'MCAP_score', 'DamagePredCount_mean',
                 'vest_score_mean', 'MVP_score_mean', 'MPC_score_mean', 'DANN_score_mean', 'GenoCanyon_score_mean',
                 'Integrated_fitCons_score_mean', 'GM12878_fitCons_score_mean', 'H1_hESC_fitCons_score_mean',
                 'HUVEC_fitCons_score_mean', 'LINSIGHT_mean', 'GERP_NR_mean', 'GERP_RS_mean', 'PhyloP100way_vertebrate_mean',
                 'PhyloP30way_mammalian_mean', 'PhyloP17way_primate_mean', 'PhastCons100way_vertebrate_mean',
                 'PhastCons30way_mammalian_mean', 'PhastCons17way_primate_mean', 'RVIS_ExAC_005_1', 'RVIS_ExAC_005_2',
                 'REVEL_score', 'Roadmap_SRS004212', 'E003_DNase', 'E003_H3K27ac', 'E003_H3K27me3', 'E003_H3K36me3',
                 'E003_H3K4me1_signal', 'E003_H3K4me3_signal', 'E003_H3K9me3_signal', 'E003_Methylation_signal',
                 'ENCFF225MAO_signal', 'EncodeUwRepliSeqGm12878WaveSignal_signal', 'phyloP100way_signal', '3utr_overlap',
                 '5utr_overlap', 'CDS_overlap', 'Prom_overlap', 'SS_overlap', 'TAD_overlap', 'sensitive_overlap',
                 'conserved_overlap', 'Heterochromatin_overlap',
                 ...
                 'ExonicFunc.refGene_None', 'ExonicFunc.refGene_frameshift_insertion', 'ExonicFunc.refGene_nonframeshift_insertion',
                 'ExonicFunc.refGene_startloss', 'ExonicFunc.refGene_stopgain', 'ExonicFunc.refGene_unknown',
...
def extract_features(reference, cadd_dir, mcap_dir, dbnsfp_dir, revel_dir, gene_dir, bw_dir, overlap_dir, step, variationINFO):
    ...
    try:
        ...
        snpREF = fasta.fetch(tmpchrom, pos, pos + sv_len)
        snpREF = snpREF.upper()
        snpALT = alt[1:]
        ...
        bwFeatures_scores = featuresInbwFiles(bw_dir, chrom, pos, pos + sv_len + 1)
        for i in range(len(bwFeatures_scores)):
            featuresValue4currentSV.append(round(bwFeatures_scores[i], 6))

        overlapFeatures = featuresForOverlap(overlap_dir, chrom, pos, pos + sv_len + 1)
        ...
        print('chr' + str(chrom) + ', pos ' + str(pos) + ' insertion variation features calculation completed')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate a feature matrix from insertion SV data')
