...
def get_sv_mcap_file_list(chrom, sv_start, sv_end, step, ref_length, mcap_dir):
    ...
    while bed_start <= sv_end:
        ...
        file_name = os.path.join(mcap_dir, 'mcap_' + chrom + '-' + str(bed_start + 1) + '-' + str(end) + '.txt')
...
def get_chr_list(reference):
    ...
    for chrom in chrom_list:
        chr_list.append(['chr' + chrom, 1, refFasta.get_reference_length('chr' + chrom), 'mcap_'+chrom+'.txt'])
    return chr_list
...
def para_chunking(chrom):
    ...
    with open(chrom_file, 'r') as mcap_reader:
        flag = True
        tmpLine = ''
        for chunk in chunk_list:
            ...
            bed_end = chunk[2]
            print(chunk)
            chunk_file_name = 'mcap_' + chromName + '-' + str(bed_start) + '-' + str(bed_end) + '.txt'

            with open(chunk_file_name, 'w') as chunk_writer:
                while True:
                    if flag == True:
                        line = mcap_reader.readline()
                        tmpLine = line
                    else:
                        line = tmpLine
                    if line:
